              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {items.map((skill) => (
                    <span
                      key={skill.name}
                      className="rounded-lg border border-[var(--border)] bg-[var(--card-bg)] px-3 py-1.5 text-sm text-[var(--text-2)] transition-colors hover:border-[var(--accent)] hover:text-[var(--text)]"
                    >
                      {skill.name}
                    </span>
                  ))}
        ...
        <div className="mt-5 rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {CLOUD_TOOLS.map((tool) => (
              <span
                key={tool.name}
                className="rounded-full border border-[var(--border)] bg-[var(--bg)] px-3 py-1 text-xs text-[var(--text-2)]"
              >
                {tool.name}
              </span>
            ))}
